...
from models.encoder import PoolEncoder, TEDClassifier
...
class SEARAgent:
    ...
    def __init__(self, obs_shape, action_shape, device, lr, latent_dim,
        ...
        mask_shape = [obs_shape[0] // 3, obs_shape[1], obs_shape[2]]
        decoder_input_dim = int(latent_dim / 2) if split_latent else latent_dim
        self.mask_encoder = PoolEncoder(mask_shape, repr_dim=decoder_input_dim).to(device)
        ...
        self.mask_ted_classifier = TEDClassifier(decoder_input_dim).to(self.device)
        # if self.ted:
        ...
        mask_params = list(self.mask_encoder.parameters()) + list(self.mask_ted_classifier.parameters())
        self.mask_enc_opt = torch.optim.Adam(mask_params, lr=mask_lr)
        # if self.ted:
    ...
    def train(self, training=True):
        ...
        self.mask_encoder.train(training)
    ...
    def update_critic_and_decoders(self, obs, encoded_obs, action, reward,
                                   ...
                                   robot_masks, obs_shape):
        metrics = {}
        ted_loss = 0
        with torch.no_grad():
            stddev = utils.schedule(self.stddev_schedule, step)
            _, f2_next = self.get_latent_splits(encoded_next_obs)
        ...
        encoded_mask_obs = self.mask_encoder(robot_masks)
        
        # _,f2_same_episode = self.get_latent_splits(encoded_same_episode_obs)
        mask_loss = self.update_mask_enc(encoded_mask_obs, f2, f1.detach(), obs_shape)
        # if self.ted:
        ...
        # loss = critic_loss + mask_loss * self.mask_loss_coef + ted_loss * self.ted_loss_coeff
        loss =  critic_loss + mask_loss * self.mask_loss_coef 
        # if self.ted:
        #     self.ted_optimizer.zero_grad()
        self.encoder_opt.zero_grad(set_to_none=True)
        self.mask_enc_opt.zero_grad(set_to_none=True)
    ...
    def load_pretrained_weights(self, pretrain_path, just_encoder_decoders):
        ...
        self.mask_encoder.load_state_dict(
            pretrained_agent.mask_encoder.state_dict())
