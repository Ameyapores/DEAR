...
class SegmentationToRobotMaskWrapper(dm_env.Environment):
    ...
    def _transform_observation(self, time_step):
        ...
        # for i in geoms_ids:
        #     name = self._env.sim.model.geom_id2name(i)
        #     if "robot" in name:
        #         robot_mask[ids == i] = (255, 0, 0)
...
class Render_Wrapper:
    ...
    def render(self, *args, **kwargs):
        # Check if 'segmentation' is in kwargs
        if 'segmentation' in kwargs:
            # Handle 'segmentation' argument
            segmentation = kwargs.pop('segmentation')
            # Call render function with remaining arguments
            result = self.render_fn(*args, **kwargs)
            # Do something with segmentation...
            return result
        else:
            # Call render function normally
            return self.render_fn(*args, **kwargs)
