...
class ReplayBuffer(IterableDataset):
    ...
    def __init__(self, replay_dir, max_size, num_workers, nstep, discount,
                 fetch_every, save_buffer_snapshot, has_success_metric, ted):
                # fetch_every, save_buffer_snapshot, has_success_metric, obs_spec, image_pad):
        self._nesting_delimiter = "."
        self._replay_dir = replay_dir
        self._size = 0
    ...
    def _store_episode(self, eps_fn):
        ...
        self.idx = (self.idx + 1) % self._max_size
    ...
    def _sample(self):
        ...
        idxs = np.random.randint(0, episode_len(episode) - self._nstep + 1) + 1
        episode = unflatten_dict(episode, self._nesting_delimiter)
        obs = get_timestep_from_nested_dict(episode["observation"], idxs - 1)
        action = episode["action"][idxs]
        next_obs = get_timestep_from_nested_dict(episode["observation"],
                                                 idxs + self._nstep - 1)
        if self.has_success_metric:
            reward = np.zeros_like(episode["reward"]["reward"][idxs])
        else:
            reward = np.zeros_like(episode["reward"][idxs])
        discount = np.ones_like(episode["discount"][idxs])
        for i in range(self._nstep):
            if self.has_success_metric:
                step_reward = episode["reward"]["reward"][idxs + i]
            else:
                step_reward = episode["reward"][idxs + i]
            reward += discount * step_reward
            discount *= episode["discount"][idxs + i] * self._discount
        return (obs, action, reward, discount, next_obs)
    
    def _sample_ted(self):
        try:
            self._try_fetch()
        except Exception:
            traceback.print_exc()
        ...
        # add +1 for the first dummy transition
        idxs = np.random.randint(0, episode_len(episode) - self._nstep + 1) + 1
        idxs2 = np.random.randint(0, episode_len(episode) - self._nstep + 1) + 1
        while idxs2 == idxs or idxs2 == idxs+1 or idxs2 == idxs-1:
            idxs2 = np.random.randint(0, episode_len(episode) - self._nstep + 1) + 1
        episode = unflatten_dict(episode, self._nesting_delimiter)
        obs = get_timestep_from_nested_dict(episode["observation"], idxs - 1)
        action = episode["action"][idxs]
        next_obs = get_timestep_from_nested_dict(episode["observation"],
                                                 idxs + self._nstep - 1)
        same_episode_obs = get_timestep_from_nested_dict(episode["observation"], idxs2 - 1)
        if self.ted:
            same_episode_obs = get_timestep_from_nested_dict(episode["observation"], idxs2 - 1)
        else:
        ...
        return (obs, action, reward, discount, next_obs, same_episode_obs)

    def __iter__(self):
        while True:
            if self.ted:
                yield self._sample_ted()
            else:
                yield self._sample()
            # yield self._sample()
...
def make_replay_loader(replay_dir, max_size, batch_size, num_workers,
                    ...
                    #    has_success_metric, ted, obs_spec, image_pad):
                        has_success_metric, ted):
    ...
    iterable = ReplayBuffer(replay_dir,
                            ...
                            # has_success_metric=has_success_metric, ted=ted, obs_spec=obs_spec, image_pad=image_pad)
                            has_success_metric=has_success_metric, ted=ted)
